...
(function () {
    ...
    function vText(input, fieldName) {
        if (!input) return true;
        const v = input.value.trim();
        if (!v) { setError(input, fieldName + ' is required.'); return false; }
        clearError(input); return true;
    }
    ...
    function setupForm(form) {
        ...
        const subjectEl = q('subject') || form.querySelector('input[name="subject"]');
        const messageEl = q('message') || form.querySelector('textarea[name="message"]');
        ...
        if (subjectEl) {
            subjectEl.addEventListener('blur', function () { vText(subjectEl, 'Subject'); });
            subjectEl.addEventListener('input', function () { if (subjectEl.value.trim()) clearError(subjectEl); });
        }
        if (messageEl) {
            messageEl.addEventListener('blur', function () { vText(messageEl, 'Message'); });
            messageEl.addEventListener('input', function () { if (messageEl.value.trim()) clearError(messageEl); });
        }
        ...
        function validateAll(withPopup) {
            ...
            if (subjectEl && !vText(subjectEl, 'Subject')) ok = false;
            if (messageEl && !vText(messageEl, 'Message')) ok = false;
            return ok;
        }
